...
export default function TenDayForecast({ data }: TenDayForecastProps) {
  ...
  const temperatures = data.list.map((item: ForecastData) => ({
    min: item.temp.main.temp_min,  // Accessing the min temp inside the `temp` object
    max: item.temp.main.temp_max,  // Accessing the max temp inside the `temp` object
  }))
              ...
              <div className="flex w-[60%] flex-row gap-2 overflow-hidden">
                <div className="flex w-full select-none flex-row items-center justify-between gap-2 pr-2 text-sm">
                  <p className="flex w-[3rem] min-w-fit justify-end text-neutral-600 dark:text-neutral-400">
                    {Math.floor(item.temp.main.temp_min)}&deg;
                  </p>
                  <TemperatureRange
                    min={minTemperature}
                    max={maxTemperature}
                    value={[item.temp.main.temp_min, item.temp.main.temp_max]}
                  />
                  <p className="flex w-[3rem] min-w-fit justify-end">
                    {Math.floor(item.temp.main.temp_max)}&deg;
                  </p>
                </div>
              </div>
            </div>
